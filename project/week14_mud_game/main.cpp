...
vector<vector<int>> Map(const string& fileName);

// 메인  함수
int main() {
	// 0은 빈 공간, 1은 아이템, 2는 적, 3은 포션, 4는 목적지

	vector<vector<int>> map = Map("map.txt");
	if (map.empty()) {
        cerr << "맵 데이터를 로드할 수 없습니다. 프로그램을 종료합니다." << endl;
        return 1;
    }
	...
	int choice;
        ...
        if(choice == 1){
			...
			cerr << "잘못된 선택입니다. 프로그램을 종료합니다." << endl;
    			return 1;
        }
	...
	while (true) { // 사용자에게 계속 입력받기 위해 무한 루프
		...
		|| user_input == "left" || user_input == "right") {
			...
			cerr << "잘못된 입력입니다. 사용 가능한 명령어를 확인하세요." << endl;
			continue;
		}
...
vector<vector<int>> Map(const string& fileName) {
    ...
    while (getline(file, line)) {
        ...
        for (char c : line) {
            if (isdigit(c)) {
                row.push_back(c - '0'); // 문자 숫자를 정수로 변환
            }
        }
        if (!row.empty()) {
            map.push_back(row);
